...
/**
 * interrupt occured two times for each operation
 * push: make code
 * pull: break code
 *
 * make code = break code | 8
 */
typedef enum {
    KEY_NONE,
    KEY_ESC,
...
void keymap_handler(int idt_index) {
    uchar ext = 2; // keymap state index

    // tell 8259c, the interrupt has solved
    send_eoi(idt_index);
    ...
    // get ASCII
    char ch = 0;
    // [/?] need to deal with in a special way
