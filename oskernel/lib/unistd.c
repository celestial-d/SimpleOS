...
void uactive_d_shell() {
    __asm__ __volatile__ ("int 0x80"::"a"(__NR_active_d_shell));
}
...
size_t fread(void *ptr, size_t size, FILE *stream) {
    ...
    __asm__ __volatile__ ("int 0x80":"=a"(ret):"0"(__NR_fread), "b"(ptr), "c"(size), "d"(stream));
...
size_t fwrite(const void *ptr, size_t size, FILE *stream) {
    ...
    __asm__ __volatile__ ("int 0x80":"=a"(ret):"0"(__NR_fwrite), "b"(ptr), "c"(size), "d"(stream));
